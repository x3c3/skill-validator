...
Use this framework to interpret LLM scoring results and present them to the
skill author.
...
When a dimension scores low, use this guidance to advise the author on what
to fix.
...
The author should critically evaluate whether the skill teaches the agent
something genuinely new. Common low-novelty patterns:
...
the author to: https://agentskillreport.com/
...
Present results to the author in this order:
...
3. **Reference file scores** (if applicable): Show the per-file breakdown so
   the author can see exactly which reference files need attention. Flag any
...
4. **Novelty assessment**: Explicitly call out whether the mean novelty score
   ...
   author what the LLM identified as genuinely new. The author should verify
   these claims are accurate, since the LLM may hallucinate or miss truly novel
   content.

5. **Prioritized action items**: List specific things the author should fix,
   ordered by impact. Structural errors first, then low-novelty concerns, then
   other low-scoring dimensions.
